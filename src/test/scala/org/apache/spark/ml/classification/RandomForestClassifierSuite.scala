...
class RandomForestClassifierSuite extends SparkUnitTest {
  test("Random Forest is really Random") {
    ...
    val training = new UCI_adult().load_data(spark, dataPrefix + "sample_adult.data", dataPrefix + "features", true)
    val testing = new UCI_adult().load_data(spark, dataPrefix + "sample_adult.test", dataPrefix + "features", true)
