...
class GCBoostedTreeClassifier(override val uid: String)
  ...
  GCBoostedTreeClassifier.this.type = set(cacheNodeId, value)

  override def setMaxMemoryInMB(value: Int):
  GCBoostedTreeClassifier.this.type = set(maxMemoryInMB, value)
...
private[ml] class GCBoostedTreeClassificationModel (
  ...
  override def copy(extra: ParamMap): GCBoostedTreeClassificationModel = {
    copyValues(
      new GCBoostedTreeClassificationModel(uid, scanModel, cascadeForest, numClasses), extra)
  }
...
object GCBoostedTreeClassificationModel extends MLReadable[GCBoostedTreeClassificationModel] {
  override def read: MLReader[GCBoostedTreeClassificationModel] =
    new GCBoostedTreeClassificationModelReader
