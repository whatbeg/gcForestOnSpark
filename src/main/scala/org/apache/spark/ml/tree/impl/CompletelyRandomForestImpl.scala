...
private [spark] object CompletelyRandomForestImpl extends Logging {
  ...
  /**
    ...
    */
  private[tree] def binsToRandomSplit(binAggregates: DTStatsAggregator,
                                      splits: Array[Array[Split]],
  ...
  private[tree] def selectNodesToSplit(nodeStack: mutable.Stack[(Int, LearningNode)],
    ...
    while (nodeStack.nonEmpty && (memUsage < maxMemoryUsage || memUsage == 0)) {
      ...
      val nodeMemUsage = CompletelyRandomForestImpl.aggregateSizeForNode(metadata, featureSubset) * 8L
