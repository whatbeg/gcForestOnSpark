...
private[tree] class LearningNode(
  ...
  override def toString: String = s"left: ${leftChild.getOrElse(0).toString}," +
    s" right: ${rightChild.getOrElse(0).toString}," +
    s" split: ${if (split.getOrElse(None) == None) "None" else split.get.featureIndex}," +
    s" impurity: ${stats.impurity}, stats: ${stats.impurityCalculator.stats.mkString(",")}"
