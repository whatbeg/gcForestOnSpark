...
import org.apache.spark.ml.evaluation.{gcForestEvaluator, Accuracy, Metric}
...
private[spark] object GCBoostedTreeImpl extends Logging {
  ...
  (DataFrame, Metric, GradientBoostingClassificationModel) = {
    ...
    splits.zipWithIndex.foreach {
      case ((t, v), splitIndex) =>
        ...
        if (!isScan) {
          val val_acc = gcForestEvaluator.evaluate(val_result)
  ...
  def cvClassVectorGeneratorWithValidation(
      training: Dataset[_],
      testing: Dataset[_],
      gbtc: GradientBoostingTreeClassifier,
      numFolds: Int,
      seed: Long,
      strategy: GCForestStrategy,
      isScan: Boolean,
  ...
  (DataFrame, DataFrame, Metric, Metric, Array[GradientBoostingClassificationModel]) = {
    ...
    val models = splits.zipWithIndex.map {
      case ((t, v), splitIndex) =>
        val trainingDataset = sparkSession.createDataFrame(t, schema)
        val validationDataset = sparkSession.createDataFrame(v, schema)
        val model = gbtc.fit(trainingDataset)
    ...
    out_test = out_test.withColumn(strategy.featuresCol,
      UDF.mergeVectorForKfold(3)(Range(0, numFolds)
        .map(k => col(strategy.featuresCol + s"$k")): _*))
      .select(strategy.instanceCol, strategy.labelCol, strategy.featuresCol)
    val test_metric = if (!isScan) gcForestEvaluator.evaluate(out_test) else new Accuracy(0, 0)
    ...
    if (strategy.dataStyle == "Img" && strategy.multiScanWindow.length > 0) {
      ...
      Range(0, strategy.multiScanWindow.length / 2).foreach { i =>
        ...
        var (rfcFeature, _, rfcModel) =
          GCForestImpl.cvClassVectorGenerator(windowInstances, rfc,
            strategy.numFolds, strategy.seed, strategy, isScan = true, "Scan 1")
        rfcFeature = rfcFeature.withColumn(strategy.forestIdCol, lit(1))
        ...
        var (crfcFeature, _, crfcModel) =
          GCForestImpl.cvClassVectorGenerator(windowInstances, crfc,
            strategy.numFolds, strategy.seed, strategy, isScan = true, "Scan 2")
        crfcFeature = crfcFeature.withColumn(strategy.forestIdCol, lit(2))
          .withColumn(strategy.scanCol, lit(i))
      ...
      strategy.multiScanWindow.indices.foreach { i => // for each window
        ...
        val rfc =
          genRFClassifier("rfc", strategy, isScan = true, 0)
        var (rfcFeature, _, rfcModel) =
          GCForestImpl.cvClassVectorGenerator(windowInstances,
            rfc, strategy.numFolds, strategy.seed, strategy, isScan = true, "Scan 1")
        rfcFeature = rfcFeature.withColumn(strategy.forestIdCol, lit(1))
          .withColumn(strategy.scanCol, lit(i))
  ...
  def train(
    ...
    while (!reachMaxLayer) {
      ...
      val randomForests = Range(0, 8).map ( it =>
        genGBTClassifier("gbt", strategy, isScan = false, rng.nextInt + it))
  ...
  private def trainWithValidation(
    ...
    while (!reachMaxLayer) {
      ...
      lastPrediction_test = sparkSession.createDataFrame(predictRDDs(1), schema)
        .persist(StorageLevel.MEMORY_ONLY_SER)
      val outOfRounds =
        (strategy.earlyStopByTest &&
          layer_id - opt_layer_id_test >= strategy.earlyStoppingRounds) ||
          (!strategy.earlyStopByTest &&
            layer_id - opt_layer_id_train >= strategy.earlyStoppingRounds)
      if (outOfRounds) {
        println(s"[$getNowTime] " +
          s"[Result][Optimal Level Detected] opt_layer_id = " +
