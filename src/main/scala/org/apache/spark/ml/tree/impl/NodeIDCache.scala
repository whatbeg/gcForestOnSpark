...
/**
  ...
  */
private[tree] case class NodeIDUpdater(split: Split, nodeIndex: Int) {
...
private[spark] class NodeIDCache(
  ...
  def updateNodeIndices(
    ...
    val nodeIdUpdatersBc = data.sparkContext.broadcast(nodeIdUpdaters)
    prevNodeIdsForInstances = nodeIdsForInstances
    nodeIdsForInstances = data.zip(nodeIdsForInstances).map { case (point, ids) =>
      var treeId = 0
      while (treeId < nodeIdUpdatersBc.value.length) {
        val nodeIdUpdater = nodeIdUpdatersBc.value(treeId).getOrElse(ids(treeId), null)
