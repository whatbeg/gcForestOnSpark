...
#include <malloc.h>
#include <jni.h>
#include "core_JavaNativeInterface.h"
...
JNIEXPORT jdoubleArray JNICALL Java_core_JavaNativeInterface_binToBestSplit
  (JNIEnv *env, jobject thisObj, jdoubleArray inImpurityStats, jdoubleArray inallStats,
   jintArray infeatureOffset, jint nfeatureOffset, jint numSplits, jchar impurity, jint statSize,
    jint featureIndexIdx, jint minInsPerNode, jdouble minInfoGain) {

    jdouble *ImpurityStats = (*env)->GetDoubleArrayElements(env, inImpurityStats, NULL);
    jdouble *allStats = (*env)->GetDoubleArrayElements(env, inallStats, NULL);
    jint *featureOffset = (*env)->GetIntArrayElements(env, infeatureOffset, NULL);
    ...
    (*env)->ReleaseDoubleArrayElements(env, inImpurityStats, ImpurityStats, 0); // release pointer at begin
    (*env)->ReleaseDoubleArrayElements(env, inallStats, allStats, 0);
    (*env)->ReleaseIntArrayElements(env, infeatureOffset, featureOffset, 0);
    double retSplitAndStats[4 + statSize * 3];
    memset(retSplitAndStats, -1, sizeof(retSplitAndStats));
    ...
    jdoubleArray outJniRetSplitAndStats = (*env)->NewDoubleArray(env, 4 + statSize * 3); // allocate
    if (NULL == outJniRetSplitAndStats) return NULL;
    (*env)->SetDoubleArrayRegion(env, outJniRetSplitAndStats, 0, 4 + statSize * 3, retSplitAndStats); // copy
    return outJniRetSplitAndStats;
}